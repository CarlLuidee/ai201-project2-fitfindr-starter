...
def handle_query(user_query: str, wardrobe_choice: str) -> tuple[str, str, str]:
    ...
    # Step 2 — Select wardrobe based on the radio button choice
    wardrobe = (
        get_empty_wardrobe()
    ...
    # Step 4 — Surface any early-exit error in the first panel
    if session["error"]:
        return session["error"], "", ""

    # Step 5 — Format the selected listing into a readable string
    item: dict = session["selected_item"]  # type: ignore[assignment]

    brand_str     = f"Brand: {item['brand']}\n" if item.get("brand") else ""
    colors_str    = ", ".join(item.get("colors", []))
    tags_str      = ", ".join(item.get("style_tags", []))
    condition_str = item.get("condition", "").capitalize()
    platform_str  = item.get("platform", "").capitalize()
